...
create_clock -name sys_clk_pin -period 125.00 -waveform {0 5} [get_ports {clk_i}];
create_clock -add -name jtag_clk_pin -period 100.00 -waveform {0 5} [get_ports {jtag_tck_i}];
create_clock -add -name usb_clk_pin -period 10.00 -waveform {0 5} [get_ports {usb_clk_i}]


set_input_delay -clock usb_clk_pin 2.0 [get_ports cw_usb_cs_ni]
set_input_delay -clock usb_clk_pin 2.0 [get_ports cw_usb_rd_ni]
set_input_delay -clock usb_clk_pin 2.0 [get_ports cw_usb_we_ni]
set_input_delay -clock usb_clk_pin 2.0 [get_ports cw_usb_data_io]
set_input_delay -clock usb_clk_pin 2.0 [get_ports cw_usb_addr_i]

# read data will be grabbed one cycle later so no need to constrain:
set_output_delay -clock usb_clk_pin 0.0 [get_ports cw_usb_data_io]
set_false_path -to [get_ports cw_usb_data_io]
