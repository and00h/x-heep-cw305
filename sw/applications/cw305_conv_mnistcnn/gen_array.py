...
CONV_LAYERS = [
    ...
    {"id": 2, "name": "fc1", "in_shape": (1,1,1,1568), "out_shape": (1,1,1,16), "strides": (1,1), "pads": (0,0,0,0), "kernel_shape": (1,1,1,1), "dilations": (1,1), "group": 1},
    {"id": 3, "name": "fc2", "in_shape": (1,1,1,16), "out_shape": (1,1,1,10), "strides": (1,1), "pads": (0,0,0,0), "kernel_shape": (1,1,1,1), "dilations": (1,1), "group": 1} 
]


GLITCH_FULL = len(sys.argv) > 2 and int(sys.argv[2]) == 1
SECOND_STEP = len(sys.argv) > 3 and int(sys.argv[3]) == 1
SUFFIX = sys.argv[4] if len(sys.argv) > 4 else None
LAYER_IDX = 0 if GLITCH_FULL else int(sys.argv[1])
...
INPUT_FILE   = f"tensors_in/{LAYER_NAME}_in{(SUFFIX or '_b') if SECOND_STEP else ''}.npy" if GLITCH_FULL == 0 else "tensors_in/conv1_in_full.npy"
...
print(f"File '{filename}' generated with shapes: X{X_SHAPE}")
