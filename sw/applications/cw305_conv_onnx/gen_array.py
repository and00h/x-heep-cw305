...
X_SHAPE = (1, IN_CHANNELS, SPATIAL_SIZE, SPATIAL_SIZE)
W_SHAPE = (OUT_CHANNELS, IN_CHANNELS, KERNEL_SIZE, KERNEL_SIZE)
O_SHAPE = (OUT_CHANNELS, SPATIAL_SIZE // STRIDE, SPATIAL_SIZE // STRIDE)
...
x_data = np.maximum(0, np.load("tensors_in/layer3_2_conv2_in_b.npy").astype(np.float32))
...
with open(filename, "w") as f:
    ...
    f.write(f"#define SPATIAL_SIZE {SPATIAL_SIZE}\n")
    f.write(f"#define OUT_SPATIAL_SIZE {SPATIAL_SIZE // STRIDE}\n")
